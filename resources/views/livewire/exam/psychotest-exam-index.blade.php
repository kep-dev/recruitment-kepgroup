...
<div class="w-full px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    ...
    <div class="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
        <h2 class="text-2xl font-bold md:text-4xl md:leading-tight dark:text-white">
            {{ $applicantTest->JobVacancyTest->name }}</h2>
        <p class="mt-1 text-gray-600 dark:text-neutral-400">{{ $applicantTest->JobVacancyTest->jobVacancy->title }}</p>
    </div>
    <!-- End Title -->
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($this->testItems as $test)
            <div
                ...
                <div class="flex items-center gap-x-4">
                    ...
                    <div class="grow">
                        ...
                        <h3 class="font-medium text-gray-800 dark:text-neutral-200 text-uppercase">
                            {{ $test->psychotestForm->name }}
                        </h3>
                    </div>
                </div>

                <p class="mt-3 text-gray-500 dark:text-neutral-500">
                    {{ $test->psychotestForm->description }}
                </p>
                ...
                <div class="mt-3 space-x-1">
                    <button type="button"
                        ...
                        <span
                            class="inline-flex items-center py-0.5 px-1.5 rounded-full text-xs font-medium bg-blue-500 text-white">{{ $test->duration_minutes }}</span>
                        Menit
                    </button>
                    <button type="button"
                        class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 disabled:opacity-50 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800">
                        <span
                            class="inline-flex items-center py-0.5 px-1.5 rounded-full text-xs font-medium bg-blue-500 text-white">{{ $test->psychotestForm->questions->count() }}</span>
                        Soal
                    </button>
                    {{-- <button type="button" class="" @disabled(in_array($jobVacancyTestItem->id, $applicantTestAttempts)) @class([
                    ...
                    <button type="button" @disabled(in_array($test->psychotest_form_id, $attempts))
                        ...
                        @click="$wire.startTest('{{ $test->id }}', '{{ $test->psychotest_form_id }}')">
                        <span wire:loading.remove>
                            {{ in_array($test->psychotest_form_id, $attempts) ? 'Selesai' : 'Mulai' }}
                        </span>
                        <span wire:loading>
                            Memuat soal anda....
                        </span>
